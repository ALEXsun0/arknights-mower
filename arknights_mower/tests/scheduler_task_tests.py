...
class TestScheduling(unittest.TestCase):
    ...
    def init_low_priority_group_opdata(self):
        agent_base_config = PlanConfig("", "", "")
        plan_config = {
            "central": [
                Room("夕", "感知", ["麒麟R夜刀"]),
    ...
    def test_available_free_ignores_low_priority_high_occupants(self):
        op_data = self.init_low_priority_group_opdata()
    ...
    def test_plan_metadata_ignores_low_priority_group_dorms(self):
        op_data = self.init_low_priority_group_opdata()
        op_data.operators["夕"].resting_priority = "low"
        op_data.operators["夕"].current_room = "dormitory_1"
        op_data.operators["夕"].current_index = 2
